...
class RepositorioBancoDados:
    ...
    def inserir_demonstracoes(self, dados: List[Dict], arquivo_origem: str = None):
        if not self.conexao:
            return None
        ...
        try:
            ...
            for idx, registro in enumerate(dados):
                # Filtrar apenas contas que começam com '4' (despesas) e têm 9 dígitos
                cd_conta = str(registro.get('CD_CONTA_CONTABIL', '')).strip()
                if not cd_conta.startswith('4') or len(cd_conta) != 9:
                    continue  # Pular registros que não são despesas ou não têm 9 dígitos
