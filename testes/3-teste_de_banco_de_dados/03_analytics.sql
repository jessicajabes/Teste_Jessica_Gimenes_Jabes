-- QUERIES ANALITICAS - TESTE 3
-- Analise de Operadoras de Saude Suplementar
...
WITH base AS (
    SELECT 'SEM DEDUCAO' AS tipo_despesa, reg_ans, ano, trimestre, SUM(valor_despesas) AS valor_trim
    ...
    SELECT 'COM DEDUCAO' AS tipo_despesa, reg_ans, ano, trimestre, SUM(valor_despesas) AS valor_trim
...
FROM crescimento
WHERE rn <= 5
ORDER BY tipo_despesa DESC, crescimento_percentual DESC;


-- ============================================================================
-- Query 2: Distribuicao de Despesas por UF (Top 5 por tipo)
-- ============================================================================

WITH despesas_por_uf AS (
    SELECT 'SEM DEDUCAO' AS tipo_despesa, o.uf, cd.reg_ans, SUM(cd.valor_despesas) AS total
    ...
    SELECT 'COM DEDUCAO' AS tipo_despesa, o.uf, cd.reg_ans, SUM(cd.valor_despesas) AS total
...
FROM agregado
WHERE rn <= 5
ORDER BY tipo_despesa DESC, total_despesas_uf DESC;
...
WITH base AS (
    SELECT 'SEM DEDUCAO' AS tipo_despesa, reg_ans, ano, trimestre, SUM(valor_despesas) AS total_trim
    FROM consolidados_despesas
    GROUP BY reg_ans, ano, trimestre
    
    UNION ALL
    
    SELECT 'COM DEDUCAO' AS tipo_despesa, reg_ans, ano, trimestre, SUM(valor_despesas) AS total_trim
    FROM consolidados_despesas_c_deducoes
    GROUP BY reg_ans, ano, trimestre
),
media_por_tipo AS (
